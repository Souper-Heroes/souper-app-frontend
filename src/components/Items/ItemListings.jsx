...
function ItemListings({
  ...
  useEffect(() => {
    if (!user.loading && user.postcode.length) {
      createSlider();
  ...
  useEffect(() => {
    ...
    if (search.totalCount > filters.limit) {
      ...
      pages.push({
        active: false,
        text: 'PREV',
        onClick: event => onPageChangeHandler(event, 'prev', totalPages)
      });
      ...
      for (let i = 1; i <= totalPages; i++) {
        pages.push({
          active: page === i,
          text: i,
          onClick: event => onPageChangeHandler(event, i, totalPages)
        });
      }
      pages.push({
        active: false,
        text: 'NEXT',
        onClick: event => onPageChangeHandler(event, 'next', totalPages)
      });
      setPagination(pages);
    } else {
      setPagination([]);
    ...
    <div className={classNames(classes.main, classes.mainRaised)}>
      <div>
        <div className={classes.container}>
          {!user.loading && !user.postcode.length ? (
            <Location />
          ) : (
            <GridContainer>
                ...
                <GridItem xs={12} sm={12} md={12}>
                  ...
                  <FormControl fullWidth>
                    <Datetime
                      ...
                      value={
                        expiry.length ? moment(expiry).format('DD/MM/yyyy') : ''
                      }
                      // value={expiry}
              ...
              >
                <Paginations pages={pagination} color="rose" />
              </GridItem>
            </GridContainer>
          )}
        </div>
